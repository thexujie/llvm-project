...
bool InstrLowerer::lowerIntrinsics(Function *F) {
  ...
  for (auto *IP : InstrProfInsts) {
    if (auto *IPIS = dyn_cast<InstrProfIncrementInstStep>(IP)) {
      ...
      MadeChange = true;
    } else if (auto *IPMP = dyn_cast<InstrProfMCDCBitmapParameters>(IP)) {
      doSampling(IP);
      IPMP->eraseFromParent();
      MadeChange = true;
    } else if (auto *IPBU = dyn_cast<InstrProfMCDCTVBitmapUpdate>(IP)) {
      doSampling(IP);
      lowerMCDCTestVectorBitmapUpdate(IPBU);
      MadeChange = true;
    } else if (auto *IPTU = dyn_cast<InstrProfMCDCCondBitmapUpdate>(IP)) {
      doSampling(IP);
