...
static bool convertSwitchToCmpAndSelect(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (!initializeuniqueCasesWithoutConstants(
          SI, PHI, CommonDest, Map, UniqueResults, DefaultResult, DL, TTI, 15))
    return false;

  /// Helper function that will return true if there is no space for
  /// optimization
  auto isUnableToOptimize = [](PHINodeToCaseEntryValueMapTy &Map) -> bool {
  ...
  for (const auto &Entry : Map) {
    ...
    Value *SelectValue = foldSwitchToSelect2(ResultVector, DefaultResult, Cond,
                                             PHI, SI, Builder, DTU);
