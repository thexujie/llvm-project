...
; X86-SSE2:       # %bb.0:
; X86-SSE2-NEXT:    movl glob32, %eax
; X86-SSE2-NEXT:    movl %eax, glob32
; X86-SSE2-NEXT:    retl
;
; X86-AVX-LABEL: fmul_32g:
; X86-AVX:       # %bb.0:
; X86-AVX-NEXT:    movl glob32, %eax
; X86-AVX-NEXT:    movl %eax, glob32
; X86-AVX-NEXT:    retl
;
; X64-SSE-LABEL: fmul_32g:
; X64-SSE:       # %bb.0:
; X64-SSE-NEXT:    movl glob32(%rip), %eax
; X64-SSE-NEXT:    movl %eax, glob32(%rip)
; X64-SSE-NEXT:    retq
;
; X64-AVX-LABEL: fmul_32g:
; X64-AVX:       # %bb.0:
; X64-AVX-NEXT:    movl glob32(%rip), %eax
; X64-AVX-NEXT:    movl %eax, glob32(%rip)
; X64-AVX-NEXT:    retq
  %i = load atomic i32, ptr @glob32 monotonic, align 4
  %f = bitcast i32 %i to float
  %mul = fmul float %f, 1.000000e+00
...
; X86-SSE2-NEXT:    movl %eax, -559038737
; X86-SSE2-NEXT:    retl
;
; X86-AVX-LABEL: fmul_32imm:
; X86-AVX:       # %bb.0:
; X86-AVX-NEXT:    movl -559038737, %eax
; X86-AVX-NEXT:    movl %eax, -559038737
; X86-AVX-NEXT:    retl
;
; X64-SSE-LABEL: fmul_32imm:
; X64-SSE:       # %bb.0:
; X64-SSE-NEXT:    movl $3735928559, %eax # imm = 0xDEADBEEF
; X64-SSE-NEXT:    movl (%rax), %ecx
; X64-SSE-NEXT:    movl %ecx, (%rax)
; X64-SSE-NEXT:    retq
;
; X64-AVX-LABEL: fmul_32imm:
; X64-AVX:       # %bb.0:
; X64-AVX-NEXT:    movl $3735928559, %eax # imm = 0xDEADBEEF
; X64-AVX-NEXT:    movl (%rax), %ecx
; X64-AVX-NEXT:    movl %ecx, (%rax)
; X64-AVX-NEXT:    retq
  %i = load atomic i32, ptr inttoptr (i32 3735928559 to ptr) monotonic, align 4
  %f = bitcast i32 %i to float
  %mul = fmul float %f, 1.000000e+00
...
; X64-AVX-NEXT:    movl %eax, -{{[0-9]+}}(%rsp)
; X64-AVX-NEXT:    retq
  %ptr = alloca i32, align 4
  %load = load atomic i32, ptr %ptr acquire, align 4
  %bc0 = bitcast i32 %load to float
  %fmul = fmul float 1.000000e+00, %bc0
...
; X64-AVX-NEXT:    retq
  ...
  %load = load atomic i64, ptr %ptr acquire, align 8
  %bc0 = bitcast i64 %load to double
  %fmul = fmul double 1.000000e+00, %bc0
